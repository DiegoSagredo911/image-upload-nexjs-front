...
const ModelModify = ({
  ...
  const saveImage = async () => {
    ...
    const blob = await CompressImage(getsrc);

    GetBase64(blob).then(data => {
      axios.post(process.env.service,{userId:`${userId?userId:process.env.service.default}`,base64:data,folderSave:`${folderSave?folderSave:process.env.service.default}`,format:`${format?format:process.env.service.format}`})
      .then((data)=>{
        if (data.data.status) {
              setSrc(undefined)
              setAlert(true);
  ...
  return (
    <div className=" flex justify-center flex-wrap fixed top-0 left-0  w-full h-full outline-none overflow-x-hidden overflow-y-auto items-center  backdrop-blur-md">
      <div
        ...
        <div className="flex justify-center  h-auto max-w-lg">
...
{
  getsrc!=undefined?<img  src={getsrc} alt="imagen"/>:""
}
...
const ImageUpload = ({ config, configStyle, userId }) => {
